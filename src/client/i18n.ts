...
const ZH_DICT = {
  ...
  'wish.noTasks': '暂无下属任务',
  'wish.pageTitle': '我的愿望',
  'wish.summary': '{active} 个进行中{achieved}',
  'wish.achievedSuffix': ' · {n} 个已达成',
  ...
  'toast.alreadyClaimed': '该任务已领取过',
  'toast.undone': '打卡已撤销',
  'toast.undoneAt': '已撤销 {date} 的打卡',
  'toast.wishCreated': '已创建愿望「{title}」',
  'toast.taskCreated': '已创建任务「{name}」',
  ...
  'toast.categoryDeleted': '已删除空分类「{name}」',

  // --- 确认弹窗 ---
  'confirm.futureCheckin': '「{name}」今天不是打卡日，提前打卡 {date} 表示承诺当天完成。确认？',
  'confirm.undoToday': '确定撤销「{name}」今天的打卡吗？进度将回退。',
  'confirm.undoAt': '确定撤销「{name}」在 {date} 的打卡吗？进度将回退。',
  'confirm.futureAt': '「{name}」在 {date} 不是当天，提前打卡表示承诺当天完成。确认？',
  'confirm.deleteTask': '确定删除任务「{name}」吗？其打卡记录将一并删除，不可恢复。',
  'confirm.deleteWish': '确定删除愿望「{name}」吗？下属任务与打卡记录将一并删除，不可恢复。',
  'confirm.renameCategory': '把分类「{old}」下的全部愿望改名为「{new}」吗？（共 {count} 个）',
  'confirm.deleteEmptyCategory': '删除空分类「{name}」的自定义颜色吗？',
  ...
  'growth.rewardPrefix': '当前等级权益：{reward}',
  ...
  'growth.chart.hint': '绿色为已打卡，灰色为当日未完成缺口；悬停柱子可看逐日明细。',
  ...
  'settings.coach.gentle': '温柔型 · 暖心鼓励',
  'settings.coach.strict': '严格型 · 直接鞭策',
  'settings.coach.humorous': '幽默型 · 轻松调侃',
  ...
  'settings.pref.unavailable': '当前连接不支持持久化偏好（远程或临时模式），以下两项不可用。',
  'settings.pref.loading': '偏好加载中…',
  'settings.pref.confirmWrites': '写操作二次确认',
  'settings.pref.confirmWritesHint': '创建愿望/任务、打卡、取消打卡时弹出系统确认卡（删除始终确认）。关闭后对话中的这类操作将直接执行。',
  ...
  'detail.grid.summary': '近 {total} 个机会日：已打 {checked}，未打 {missed}，未来 {future}',
  'detail.legend.checked': '已打卡',
  'detail.legend.missed': '未打卡',
  'detail.legend.future': '未来机会日',
  'detail.grid.checked': '{date} 已打卡',
  'detail.grid.missed': '{date} 未打卡',
  'detail.grid.future': '{date} 未来机会日',
  'detail.next.title': '接下来的机会日：{dates}',
...
const EN_DICT: Record<XyKey, string> = {
  ...
  'wish.noTasks': 'No tasks yet',
  'wish.pageTitle': 'My Wishes',
  'wish.summary': '{active} active{achieved}',
  ...
  'action.checkinFuture': 'Commit to {date}',
  'action.checkinThisDay': 'Check in this day',
  'action.undoCheckin': 'Undo',
  'action.cancelCheckin': 'Cancel check-in',
  'action.expand': 'Expand details',
  'action.collapse': 'Collapse details',
  'action.createWish': '+ New wish',
  'action.createTask': '+ New task',
  'action.manageCategories': 'Categories',
  'action.askAi': 'Summarize with AI',

  'toast.claimed': 'Claimed "{name}". Time to check in!',
  'toast.checkinOk': 'Checked in',
  'toast.alreadyClaimed': 'This task was already claimed',
  ...
  'toast.categoryDeleted': 'Empty category "{name}" removed',

  'confirm.futureCheckin': '"{name}" is not due today. Checking in early for {date} means committing to finish it that day. Continue?',
  ...
  'confirm.futureAt': '{date} is not the due day of "{name}". Checking in early means committing to finish it that day. Continue?',
  'confirm.deleteTask': 'Delete task "{name}"? Its check-in history will be removed too. This cannot be undone.',
  'confirm.deleteWish': 'Delete wish "{name}"? Its tasks and check-ins will be removed too. This cannot be undone.',
  'confirm.renameCategory': 'Rename category "{old}" to "{new}" for all {count} wishes?',
  ...
  'cal.cellAria.none': '{date}, nothing scheduled',
  ...
  'growth.stat.checkinDays': 'Total check-in days',
  ...
  'growth.chart.hint': 'Green = checked in, grey = missed gap; hover a bar for daily details.',
  ...
  'memory.capNote': '{total} in total, showing the latest {shown}; narrow down with search.',
  ...
  'memory.confirmClear': 'Clear all {total} memories? This cannot be undone!',
  ...
  'settings.coach.gentle': 'Gentle · warm encouragement',
  'settings.coach.strict': 'Strict · direct push',
  'settings.coach.humorous': 'Humorous · light teasing',
  ...
  'settings.pref.unavailable': 'This connection cannot persist preferences (remote or temporary mode); the two options below are disabled.',
  'settings.pref.loading': 'Loading preferences…',
  'settings.pref.confirmWrites': 'Confirm write actions',
  'settings.pref.confirmWritesHint': 'Creating wishes/tasks, checking in or cancelling pops a confirmation card (deletes always confirm). Turn off to execute directly.',
  'settings.pref.memoryLimit': 'Memory injection limit',
  'settings.pref.memoryLimitHint': 'Max memories auto-injected per chat (5-200, default 40); saved on blur.',
  ...
  'detail.grid.summary': 'Last {total} opportunity days: {checked} checked, {missed} missed, {future} upcoming',
  'detail.legend.checked': 'Checked in',
  'detail.legend.missed': 'Missed',
  'detail.legend.future': 'Upcoming',
  'detail.grid.checked': '{date} checked in',
  'detail.grid.missed': '{date} missed',
  'detail.grid.future': '{date} upcoming opportunity',
  ...
  'err.already_checked': 'Already checked in on this date',
  'err.not_opportunity_day': 'This date is not an opportunity day',
