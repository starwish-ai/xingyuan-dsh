...
7. **承诺口径（2026-08，见 §10 决策 11）**：未领取（pending）任务处于候选池——未承诺
   ...
   **已领取且未完结**的义务——已完结（closed，达标关闭 achieved）任务不进入日程面
   ...
   未领取任务单列
   「可领取」区（任务页待领取组、开场概览、工具附带行），并约束模型**不得主动领取**
   ——领取是用户决定，误领取同误打卡。
   **今日页与日历详情面板（含今天）只呈现已领取任务**：候选池不进今日页、不进日历面板
   （2026-08 用户裁决两轮——「今日/日历」= 承诺面，待领取属计划面；领取入口收归任务页
   待领取组与对话开场概览，独立页 /xingyuan/today 与 /xingyuan/calendar 详情同口径）。
   **「长期」比率为承诺口径（2026-09 修订，见 §10 决策 11）**：愿望进度（页面卡片三态、
   wishProgress/taskCompletionRate 图、get_wish_* 工具输出）分母只计已领取任务——候选
   不进比率、不拖累完成率，删除候选也不抬高进度；**达成 = 进度 100% 且无候选**
   （`store.ts` `wishProgressFromAgg` 单一公式，写路径与读侧共用，消费侧禁止重复判定）——
   满进度而有候选 = 「待结算」（进度条满、不归档）：愿望卡附候选计数与收尾指引行、
   工具输出注明，引导用户领取继续或删除候选收尾，模型不得为促成达成主动领取或删除；
   **展示派生位单一来源**：`freshWishes` 视图随公式产出 `pendingCount`/`settled`，
   随 `/api/wishes` 与会话 `xingyuan/wish` 事件下发，页面/聊天卡/摘要一律消费下发位
   （禁止展示层重建 `progress>=100 && 有候选` 闸门）；**达成闸门翻转必须触达模型**：
   打卡/领取/更新/取消/删除（含批量）与**创建**（含批量——向已达成愿望追加候选会退档）等
   任务写操作完成后经 `wishSettleNote` 比对写入前后视图（批量入口前后视图各走
   `wishViewsBefore`/`wishSettleNotes` 单遍索引，守批量读契约），达成/进待结算/回退
   （**含待结算→进行中的分母扩大回退**，如延长截止日、领取多日候选）时回包注明并补发 wish 事件
   （卡面与回包、愿望页三方同源，收尾路径「删除候选→达成」「领取 0 天候选→直接达成」
   「追加候选→退档待结算」不静默）；非翻转但待领取数变化同样补发 wish 事件（不产回包文案）——
   卡上「N 个任务待领取」计数与页面同源，增删候选后卡不定格旧数；
   三态展示位 planning（无已领取任务）亦由 `wishProgressFromAgg` 同公式产出，
   页面/聊天卡/列表回包一律消费，禁任务状态谓词重建；分区折叠唯一收口 `foldTaskIntoAgg`。
   **用语规范（2026-09 用户裁决）**：用户可见文案与模型逐字转述面（i18n / 工具回包 /
   提示词指南 / 独立备用页 pages-html）禁内部词——「候选（任务）」→「待领取的任务」；
   「待结算」→「待收尾」；「口径」「锚点」「分母」「账本」不得出现在面向用户句子里
   （文档/注释/工具 description 的模型指令面可用）。达成庆祝 emoji「🎉」统一置行首
   （避免「…候选。🎉成长：」粘连，读侧 statusNote 行尾「已达成 🎉」为句末自然收尾可保留）；
   收尾句固定句式「领了继续，或删掉就达成」；加新状态先定口语名再落文案，禁词约束见愿望指南。
   开场概览（`xingyuan:today`）读 `freshWishes`
   而非库存 `archived`（库存位只在写路径刷新，闸门变更/跨日新鲜化下可与派生面分叉）；
   图表副标题恒标注「已领取任务」口径（无候选时的条件分支文案已取消——一个指标一个含义）；
   未来安排查询工具（`get_tasks_for_next_days` /
   `get_tasks_for_date_range`）维持计划口径并在描述/计数中标注未领取。
   月历图例 c0 文案为「无打卡安排」以对齐该
   口径（未领取机会日隐身是承诺面的有意取舍，预览由任务详情 ≤5 个未来机会日兜底）。
   改这些口径必须先改 §10 决策 11 与本条，再同步单测对拍（test/routes.test.ts、
   test/integrity.test.ts、test/charts.test.ts；闸门翻转回包/事件与用语规范另见
   test/tools.test.ts、test/prompts.test.ts）。
...
| kind | 载荷要点 |
|---|---|
| `xingyuan/wish` | op(created/updated/deleted) + 愿望快照（含 optional `pendingCount`/`settled` 派生位，旧事件回放如实降级） |
...
- taskCompletionRate 与 wishProgress 均为承诺口径：分母只计已领取任务的应打天数（与愿望
  进度同一公式族，2026-09 修订），候选不计入也不拖累；副标题恒标注「已领取任务」，
  达成判定 = 进度 100% 且无候选（候选拦达成，`wishProgressFromAgg` 单一公式）；
...
11. **承诺口径分层（2026-08 定；2026-09 修订——比率面并入承诺口径）**：未领取（pending）任务处于候选池，
    ...
    挡住；过期关闭属失败记录，留在截止日日程面如实呈现，见 §5.2 规则 7），未领取单列「可领取」
    并以约束词防模型主动领取——单列位置在任务页待领取组与对话开场概览；今日页与日历详情
    面板（含今天）不展示候选池（2026-08 用户裁决两轮：今日页→日历面板，承诺面不混候选池）；
    长期比率（愿望进度——页面卡片与 wishProgress/taskCompletionRate 图）2026-09 起同为
    **承诺口径**：分母只计已领取任务（候选不进比率、不拖累完成率；业界对照 GitHub Projects
    draft 不计进度、Scrum velocity 只计 committed；把未承诺工作计入完成率是「生产力内疚」
    反模式），**达成 = 进度 100% 且无候选**（候选拦达成：满进度而有候选 = 「待结算」，
    收尾是用户决定，模型不得代结；此闸门同时保证口径切换的迁移安全——切换只会抬高
    进度，新达标者必含候选、被闸门拦住而非意外归档）。未来安排查询工具维持计划口径。
    **实现约束：过滤统一走 `store.ts` 的 `isClaimed` 判定，进度/达成公式单一收口
    `wishProgressFromAgg`（写路径与读侧共用，读侧视图携 `pendingCount`/`settled`
    派生位下发给 API/事件/回包，taskCompletionRate 图求和走 `claimedDaysSum`，
    消费侧零重建）；任务写操作的闸门翻转经 `wishSettleNote` 回包 + 补发 wish 事件；机会日序列
...
- 无任务的愿望没有「手动标记达成」路径：达成判定 ≡ 进度 100% 且无候选任务
  （`wishProgressFromAgg` 派生，2026-09 起候选拦达成），
