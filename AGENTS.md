...
7. **承诺口径（2026-08，见 §10 决策 11）**：未领取（pending）任务处于候选池——未承诺
   ...
   **已领取且未完结**的义务——已完结（closed，达标关闭 achieved）任务不进入日程面
   ...
   未领取任务单列
   「可领取」区（任务页待领取组、开场概览、工具附带行、今天的日历详情面板保留领取
   入口），并约束模型**不得主动领取**——领取是用户决定，误领取同误打卡。
   **今日页只呈现今天的已领取义务**（待打卡组 + 已完成撤销组）：候选池不进今日页
   （2026-08 用户裁决——「今日」= 承诺面，待领取属计划面；领取入口在任务页待领取组、
   日历今天面板与对话，独立页 /xingyuan/today 同口径）。
...
11. **今日承诺口径 × 长期计划口径分层（2026-08）**：未领取（pending）任务处于候选池，
    ...
    并以约束词防模型主动领取——单列位置在任务页待领取组、日历今天面板与对话开场概览，
    今日页不展示候选池（2026-08 用户裁决：今日=承诺面）；长期尺度（愿望进度——页面卡片与愿望进度图、taskCompletionRate）
    维持含未领取的计划
