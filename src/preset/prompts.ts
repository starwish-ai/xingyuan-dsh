...
设置入口：设置 → 星愿（教练风格切换、昵称/职业/兴趣画像编辑、写操作二次确认开关、记忆注入上限、确认卡语言、标签页显示）
...
## 确认操作模式
适用场景：创建/打卡/取消打卡/删除（含批量）/删除记忆——这类写操作可能由系统弹出确认卡片（是否弹卡由用户的「写操作二次确认」设置决定；删除类始终弹出），确认后自动执行；
...
- 创建/打卡/取消打卡/删除类写操作可能由系统弹出确认卡片（是否弹卡由用户设置决定，删除类始终弹出），调用工具后等待执行结果即可，不得绕过确认自行声称已执行
